    ...
    connect() {
        ...
        this.ws.onclose = async (event) => {
            ...
            if (event.code === 1006) {
                ...
                const res = await fetch(this.url.origin + "/gloves-link/status?" + params.toString());
                if (!res.ok)
                    return console.error("[ws] Status error", res.status);
                ...
                if (status.status === 401)
                    this._handlersEmit("connect_unauthorized", status.msg);
                else if (status.status === 403)
                    this._handlersEmit("connect_forbidden", status.msg);
                else if (status.status === 500)
                    this._handlersEmit("connect_serverError", status.msg);
                return;
            }
