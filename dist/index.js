...
export class GlovesLinkClient {
    ...
    constructor(url, opts = {}) {
        ...
        this.url = new URL(url, typeof window !== "undefined"
    ...
    connect() {
        ...
        this._ws.onclose = async (event) => {
            ...
            const expDelay = Math.min(this.opts.reConnectInterval *
                this.opts.reConnectBackoffFactor ** (this._reconnectAttempts - 1), this.opts.maxReConnectDelay);
    ...
    connectSSE() {
        ...
        const sseUrl = new URL(this.opts.ssePath + this.url.pathname, this.url.origin);
        if (this.opts.token)
            sseUrl.searchParams.set("token", this.opts.token);
        if (this.opts.connectionData) {
        ...
        this._sse.onmessage = (_data) => {
            ...
            try {
                ...
                if (this.opts.logs)
                    console.warn("[sse] Invalid JSON:", _data.data);
                return;
            }
            const { evt, data } = msg;
            if (!evt || (data && !Array.isArray(data)))
    ...
    async _sendViaHTTP(payload) {
        if (!this._sseId)
            return;
        const postUrl = new URL(this.opts.ssePath + this.url.pathname, this.url.origin);
        ...
        try {
            ...
            if (this.opts.logs)
                console.warn("[sse] POST failed:", err);
        }
    }
    on(event, listener) {
    ...
    baseOn() {
        ...
        self.on("connect_unauthorized", msg => console.error("[ws] unauthorized", msg));
        self.on("connect_serverError", msg => console.error("[ws] server error", msg));
        self.on("error.valid", (...args) => console.error("[ws] error valid", ...args));
        self.on("error.spam", (...args) => console.error("[ws] error spam", ...args));
    }
}
