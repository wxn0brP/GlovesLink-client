...
> {
	...
	connect() {
		...
		if (this.opts.connectionData)
			this.url.searchParams.set(
				"data",
				JSON.stringify(this.opts.connectionData),
			);
		...
		this._ws.onclose = async (event: CloseEvent) => {
			...
			if (event.code === 1006) {
				...
				try {
					...
					if (this.opts.logs) console.error("[ws] Status error", e);
				}
			}
			...
			if (this._reconnectAttempts > this.opts.maxReConnectAttempts) {
				if (this.opts.logs)
					console.error(
						`[ws] Max reconnect attempts reached (${this.opts.maxReConnectAttempts})`,
					);
			...
			const expDelay = Math.min(
				this.opts.reConnectInterval *
					this.opts.reConnectBackoffFactor ** (this._reconnectAttempts - 1),
	...
	once<K extends EventName<InputEvents & InternalEvents>>(
		event: K,
		listener: (InputEvents & InternalEvents)[K],
	) {
